    ...
    async def find_similar_face(
        ...
        query = (
            select(
                ...
                FaceEmbedding.id,
                (1 - (FaceEmbedding.embedding.cosine_distance(embedding_list))).label(
                    "similarity"
                ),
            )
            .where(
            ...
            .order_by(FaceEmbedding.embedding.cosine_distance(embedding_list))
            .limit(limit)
        )
        ...
        for row in rows:
            ...
            if (
                emp_id not in best_by_employee
                or similarity > best_by_employee[emp_id]["similarity"]
            ):
